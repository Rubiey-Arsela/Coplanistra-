...
1. **Export/screen inconsistency**: `exportCSV()`/`exportPDF()` in `ReportsScreen.js` still read the old budgets-only variables (`totalSpent`, `totalCommitted`, `totalForecastFinal`, `budgetToDate`, `varianceToDate`) directly, bypassing the new fallback-basis variables entirely — a no-budgets client's *exported* CSV/PDF still showed A$0 even though the *on-screen* Executive Summary correctly showed real figures. Fixed: both export functions now read the same `xeroActualsBasis`/`openCommitmentsBasis`/`actualPlusCommitmentsBasis`/`forecastFinalBasis`/`varianceToDateBasis` variables as the screen, each labelled by source, with honest "Not answerable"/"No plan to compare" text instead of a fabricated 0.

2. **Critical currency-conversion bug** (found during Playwright verification): the new Executive Summary fallback tiles (and the matching PDF export) formatted real Xero-imported AUD figures with `fmtMYR()` — the app-internal formatter that multiplies by the MYR→display-currency rate (0.325 for AUD). Applied to a real Xero AUD amount, this silently shrinks it to ~32.5% of its true value (verified: real P&L expense YTD = A$47,754.40 → was rendering as "A$15.5K"). `fmtAUD()` (a formatter already in the codebase, specifically documented in `primitives.js` for this exact failure mode) renders real Xero AUD figures as-is with no conversion. Fixed: the fallback tiles + PDF export now use `fmtAUD()` for the no-budgets/Xero-sourced basis, and keep `fmtMYR()` only for the hasBudgets/internal-Budget-module basis (genuinely MYR-denominated). CSV export was already using raw unformatted numbers so was unaffected by this bug.
